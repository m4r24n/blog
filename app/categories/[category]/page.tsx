...
export default async function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  return <>
    ...
    {posts.length === 0 ? <p className="page-intro">Nothing filed here yet. This shelf is ready when a post belongs on it.</p> : <section>{posts.map((post) => <article className="post category-post" key={post.slug}><div className="meta">{formatPostDate(post.date)}<br />{post.readingTime}</div><div className="post-copy"><Link href={`/posts/${post.slug}`}><h2>{post.title}</h2></Link><p>{post.excerpt}</p></div></article>)}</section>}
  </>;
}
